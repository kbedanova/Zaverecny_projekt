...
const dnyPremiera = (filmData) => {
const premiera = dayjs(filmData.premiera)
const premieraFormat = premiera.format("D. M. YYYY")
const premieraDiff = premiera.diff(dayjs(), "days")
let vypis = `Premiera <strong>${premieraFormat}</strong>, což `
if (premieraDiff === 0){
	vypis += `je dnes.`
}
else if (premieraDiff <0){
	vypis += `bylo před ${premieraDiff * -1} `
	if (premieraDiff === -1){
		vypis += `dnem.`
	}
	else {
	...
	vypis += `bude za ${premieraDiff} `
	if (premieraDiff === 1){
		vypis += `den.`
	}
	else if (premieraDiff >1 && premieraDiff < 5){
		vypis += `dny.`
	}
	else {
		vypis += `dní.`
...
const elStars = document.querySelector(".stars");
const stars = elStars.querySelectorAll(".fa-star");

let lastClickedStarIndex = -1;
...
stars.forEach((star, index) => {
    star.addEventListener("click", () => {
		selectStar(index);
		lastClickedStarIndex = index;
	});
	star.addEventListener("mouseenter", () => {
        selectStar(index);
	});
	star.addEventListener("mouseleave", () => {
        selectStar(lastClickedStarIndex);
    });
});
...
formular.addEventListener("submit", (evt)=> {
	...
	const card = formular.parentElement

const cardText = document.createElement("p")
cardText.innerHTML = messageValue
